...
You create the private GitHub repo, push this folder, and install the phone app yourself. The version in `package.json` is the only version number. A release tag is that number with a `v` in front, such as `v1.0.0`.
